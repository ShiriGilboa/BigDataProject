...
class NBAFeedFromDB:
    ...
    def get_players(self):
        ...
        players = []
        try:
            cursor = self._db.execute(f'''select * from Players''')
        except Exception as ex:
    ...
    def get_teams(self):
        teams = []
        try:
            cursor = self._db.execute(f'''select * from Teams''')
        except Exception as ex:
    ...
    def get_games(self):
        games = []
        try:
            cursor = self._db.execute(f'''select * from Games''')
        except Exception as ex:
    ...
    def get_team_rosters(self):
        rosters = []
        try:
            cursor = self._db.execute(f'''select * from TeamRosters''')
        except Exception as ex:
    ...
    def get_all_game_lineups(self):
        lineups = []
        try:
            cursor = self._db.execute(f'''select * from GameLineUps''')
        except Exception as ex:
    ...
    def get_game_lineups(self, game_id):
        lineups = []
        cursor = self._db.execute(f'''select * from GameLineUps where GameId = {game_id}''')
    ...
    def get_all_player_scores(self):
        player_scores = []
        cursor = self._db.execute(f'''select * from GamePlayerScores''')
    ...
    def get_player_scores(self, player_id):
        player_scores = []
        cursor = self._db.execute(f'''select * from GamePlayerScores where PlayerID = {player_id}''')
    ...
    def get_all_team_scores(self):
        team_scores = []
        cursor = self._db.execute(f'''select * from GameTeamScores''')
    ...
    def get_team_scores(self, game_id):
        team_scores = []
        cursor = self._db.execute(f'''select * from GameTeamScores where GameId = {game_id}''')
